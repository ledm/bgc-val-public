...
def applyLandMask(nc,keys, **kwargs):
	try:	gridfn = kwargs['areafile']
	except:	raise AssertionError("applyLandMask:\t Needs an `areafile` kwarg to apply mask")	
	
	try:	maskname = kwargs['maskname']
	except:	maskname = 'tmask'

	try:	mask = tmask[(gridfn,maskname)]
	except:
		loadDataMask(gridfn,maskname)
		mask = tmask[(gridfn,maskname)]
			
	
	try:	shape = mask.shape
...
def applyLandMask2D(nc,keys, **kwargs):
	...
	try:	mask = tmask[(gridfn,maskname)][0]
	except:
		loadDataMask(gridfn,maskname)
...
def applyLandMask_maskInFile(nc,keys, **kwargs):
	...
	try:	mask = tmask[(nc.filename,maskname)]
	except:
	...
	try:	shape = mask.shape
	except: raise AssertionError("applyLandMask_maskInFile.py:\t Model mask not loaded correctly")
