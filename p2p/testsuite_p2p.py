...
def testsuite_p2p_noAV(	
	...
	for name in [	dataType, ]:
		...
		if len(layers) ==0:
				layers=['',]
				print "testsuite_p2p: \tWARNING: no layers provided, using defaults: ['',]"

		#####
		# Made it though the initial tests. Time to start the analysis.
		print "\ntestsuite_p2p.py:\tINFO:\tMade it though initial tests. Running:",model,jobID, year, name, layers
